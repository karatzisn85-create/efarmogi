...
if (!fs.existsSync(dataDir)) {
  try {
    ...
    // Αν δεν μπορούμε να φτιάξουμε φάκελο (π.χ. δικαιώματα), η εφαρμογή θα συνεχίσει και
    // θα αποτύχει μόνο σε λειτουργίες που απαιτούν αποθήκευση.
    console.error('Failed to create dataDir:', dataDir, e.message);
  }
}

// Δημιουργία φακέλου locks αν δεν υπάρχει
...
ipcMain.handle('export-invest-projects', async (event, options) => {
  try {
    ...
    const exportDir = app.isPackaged
      ? path.join(app.getPath('userData'), 'ektelestea_erga')
      : path.join(__dirname, '..', 'ektelestea_erga');
